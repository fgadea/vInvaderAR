...
public class shoot : MonoBehaviour {
	...
	void Update () {
		...
		#if UNITY_EDITOR
		if (((Input.GetKey ("w") && SystemInfo.supportsGyroscope) || withoutController || CrossPlatformInputManager.GetButton("shoot")) && timer >= frecuanciaDisparos) 
		#elif UNITY_ANDROID && !UNITY_EDITOR
		if ((CrossPlatformInputManager.GetButton("shoot") || withoutController || (Input.anyKey && SystemInfo.supportsGyroscope)) && timer >= frecuanciaDisparos)
		#endif
		{
			audioSource.Play ();
		...
		if(Input.GetKey(KeyCode.Return) && timer >= frecuanciaDisparos){
			timer = 0f;
			SceneManager.LoadScene ("intro");
			SceneManager.UnloadSceneAsync ("juego");
		}
